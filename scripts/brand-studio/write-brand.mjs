...
export function validateBrandPayload(body) {
	...
	const logo = assetPath(b.logo ?? '/images/dbd-cheats-logo.webp');
	const logoRaster = assetPath(b.logoRaster ?? '/images/dbd-cheats-logo.png');
	const defaultOgImage = assetPath(b.defaultOgImage ?? '/images/dbd-screenshot-gameplay.webp');
	const heroImage = assetPath(b.heroImage ?? '/images/dbd-hero-banner.webp');
	...
	const seoDefaults = {
		...
		reviewsDescription:
			'Real buyer reviews for {brand} — ESP, skill check, radar, and patch updates on Windows PC.',
	...
	const copyDefaults = {
		...
		summary:
			'{brand} is an undetected {game} cheat package for Windows PC. Includes ESP, skill check, and radar, with {antiCheat} maintenance after patches.',
		...
		reviewsIntro: 'Real feedback from {brand} buyers -- ESP, skill check, radar, and support.',
