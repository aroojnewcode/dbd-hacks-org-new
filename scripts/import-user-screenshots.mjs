...
const assetsDir =
	'C:/Users/Aman/.cursor/projects/c-Users-Aman-dbdhacks/assets';
...
const USER_SCREENSHOTS = [
	...
	},
	{
		...
		file: 'dbd-wallhack-skeleton.webp',
	},
	{
		source:
...
const REMOVE_PREFIXES = [
	'dbd-battle-royale-combat',
	'dbd-exit gate-fight',
	'dbd-cheats-aimbot',
	'dbd-cheats-cover',
	'dbd-cheats-esp-wallhack',
	'dbd-cheats-hero',
	'dbd-cheats-logo',
	'dbd-cheats-package',
	'dbd-header-art',
	'dbd-loadout-builder',
	'dbd-player-esp',
	'dbd-killer-side-combat',
	'dbd-killer-side-mode',
	'dbd-team-fight',
	'dbd-verdansk-map',
];

async function removeOldDBDImages() {
...
async function generateResponsiveVariants() {
	const heroSource = path.join(imagesDir, 'dbd-esp-player-tags.webp');
	const heroMeta = await sharp(heroSource).metadata();

	for (const width of HERO_WIDTHS) {
		if (heroMeta.width && width > heroMeta.width) continue;
		const file = `dbd-esp-player-tags-${width}w.webp`;
	...
	for (const { file } of USER_SCREENSHOTS) {
		if (file === 'dbd-esp-player-tags.webp') continue;
...
async function generateFavicons() {
	const logoPath = path.join(imagesDir, 'dbd-cheats-logo.png');
