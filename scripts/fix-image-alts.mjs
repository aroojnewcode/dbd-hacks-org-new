...
const oldImagesRe =
	/images:\s*\{\s*hero:\s*'[^']*',\s*espWallhack:\s*'[^']*',\s*aimbotCombat:\s*'[^']*',\s*teamFight:\s*'[^']*',\s*playerEsp:\s*'[^']*',\s*headerArt:\s*'[^']*',\s*cheatsPackage:\s*'[^']*',\s*rebootFight:\s*'[^']*',\s*battleRoyale:\s*'[^']*',\s*battleRoyaleIsland:\s*'[^']*'\s*\}/g;
...
for (const f of ['public/locales/en/translation.json', 'public/locales/es/translation.json']) {
	...
	if (j.hero) {
		j.hero.imageAlt = '{{brand}} — Dead by Daylight ESP and aimbot overlay';
	}
